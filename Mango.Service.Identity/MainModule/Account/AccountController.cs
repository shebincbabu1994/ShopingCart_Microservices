...
namespace IdentityServerHost.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(
            ...
            IEventService events, UserManager<ApplicationUser> userManger,
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (!_roleManger.RoleExistsAsync(model.RoleName).GetAwaiter().GetResult())
                    {
                        ...
                        await _roleManger.CreateAsync(userRole);
                    }

                    await _userManger.AddToRoleAsync(user, model.RoleName);

                    await _userManger.AddClaimsAsync(user, new Claim[]{
                            new Claim(JwtClaimTypes.Name, model.Username),
                            new Claim(JwtClaimTypes.Email, model.Email),
                            new Claim(JwtClaimTypes.FamilyName, model.FastName),
                    ...
                    {
                        var checkuser = await _userManger.FindByNameAsync(model.Username);
