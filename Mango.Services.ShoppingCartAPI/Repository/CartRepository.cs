...
namespace Mango.Services.ShoppingCartAPI.Repository
{
    public class CartRepository : ICartRepository
    {
        ...
        public async Task<CartDto> GetCartByUserId(string userId)
        {
            ...
            cart.CartDetails = _db.CartDetails.Where(p => p.CartHeaderId == cart.CartHeader.CartHeaderId).Include(p => p.Product);
        ...
        public async Task<bool> RemoveFromCart(int cartDetailsId)
        {
            ...
            .FirstOrDefaultAsync(p => p.CartDetailsId == cartDetailsId);
                int totalCountOfCartItems = _db.CartDetails.Where(p => p.CartHeaderId == cartDetails.CartHeaderId).Count();
                _db.CartDetails.Remove(cartDetails);
                if (totalCountOfCartItems == 1)
                {
                    var carHeaderToRemove = await _db.CartHeaders.FirstOrDefaultAsync(u => u.CartHeaderId == cartDetails.CartHeaderId);
